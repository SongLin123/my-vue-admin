 ...
 * @LastEditors: songlin
 * @LastEditTime: 2020-06-02 11:25:32
 * @FilePath: \d2-admin\src\store\modules\myadmin\modules\size.js
 */
import Vue from 'vue'
...
export default {
  ...
  actions: {
    ...
    apply ({ state, commit }, refresh) {
      ...
      if (refresh) {
        commit('myadmin/page/keepAliveClean', null, { root: true })
    ...
    async load ({ state, dispatch }) {
      // store 赋值
      state.value = await dispatch('myadmin/db/get', {
        dbName: 'sys',
        path: 'size.value',
